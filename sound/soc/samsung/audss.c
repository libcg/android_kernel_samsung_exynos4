...
{
	int ret = 0;
	struct clk *fout_epll, *mout_epll, *audio_bus;
	...
	if(clk_set_parent(audss.mout_audss,fout_epll)) {
		pr_err("Failed to set %s as parent of %s \n",fout_epll->name,audss.mout_audss->name);
		goto err7;
	}
	
	audio_bus = clk_get(NULL, "audio-bus");
	if (IS_ERR(audio_bus)) {
		pr_err("%s: failed to get audio-bus clk\n",__func__);
		ret = PTR_ERR(audio_bus);
		goto err7;
	}
	
	if(clk_set_parent(audio_bus,mout_epll)) {
		pr_err("Failed to set %s as parent of %s \n",mout_epll->name,audio_bus->name);
		goto err8;
	}
