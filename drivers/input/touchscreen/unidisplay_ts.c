...
static int unidisplay_ts_thread(void *kthread)
{
	...
	while (!kthread_should_stop()) {
		...
		pendown = !tsdata->pin_state(tsdata->irq);
		if (pendown) {
			u8 addr = 0x10;
...
static int unidisplay_ts_open(struct input_dev *dev)
{
	...
	ret = i2c_check_functionality(tsdata->client->adapter, I2C_FUNC_I2C);
...
{
	const struct unidisplay_ts_platform_data *pdata;
	...
	tsdata->pin_state = pdata->pin_state;
